...
export async function ensureUserProfile(uid, { email, name, photoURL }) {
  ...
  // Backfill missing profile identity fields for existing users.
  if ((!existing.name || String(existing.name).trim() === '') && safeName) {
    updates.name = safeName;
  }
  if ((!existing.email || String(existing.email).trim() === '') && safeEmail) {
